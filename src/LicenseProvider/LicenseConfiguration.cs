...
public class LicenseConfiguration
{
	...
	internal static LicenseConfiguration? CreateFromXml(ResourceXmlDocument? xml)
	{
		...
		{
			System.Diagnostics.Trace.WriteLine($"{messagePrefix} could not be parsed from '{xml.XmlString}': {xml.ParseException.Message}.");
			return null;
		}
		...
		if (String.IsNullOrWhiteSpace(assemblyIdentifier))
		{
			...
			if (String.IsNullOrWhiteSpace(assemblyIdentifier))
			{
				System.Diagnostics.Trace.WriteLine($"{messagePrefix} does not provide a proper assembly identifier. Further parsing has been aborted.");
				return null;
			}
		}
		...
		if (!success)
		{
			...
			System.Diagnostics.Trace.WriteLine($"{messagePrefix} does not provide a proper matching mode. '{nameof(AssemblyNameMatchMode.Exact)}' will be used.");
		}
		...
		if (!licenseNodes.Any())
		{
			System.Diagnostics.Trace.WriteLine($"{messagePrefix} does not provide any licenses. Further parsing has ben aborted.");
			return null;
		}
		...
		for (ushort index = 0; index < licenseNodes.Length; index++)
		{
			...
			{
				System.Diagnostics.Trace.WriteLine($"{messagePrefix} has duplicate license entries with identical version '{version}. Only the first one will be used.");
				continue;
			}
			...
			if (String.IsNullOrWhiteSpace(fileName))
			{
				...
				System.Diagnostics.Trace.WriteLine($"{messagePrefix} does not provide a proper file name. '{fileName}' will be used.");
			}
